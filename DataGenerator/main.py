...
vehicle_data = [(generate_vehicle_number(), random.choice(vehicle_models), random.choice(vehicle_brands)) for _ in
                range(10)]
...
inactive_plates_data = [(plate[0], random.choice([vehicle[0] for vehicle in vehicle_data]),
                         generate_random_date(datetime.now() - timedelta(days=365), datetime.now())) for plate in
                        plate_data[3:6]]
c.executemany('INSERT INTO InactivePlates (number, last_vehicle_number, restitutionDate) VALUES (?, ?, ?)',
              inactive_plates_data)
