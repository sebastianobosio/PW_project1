    ...
    handleAjaxRequest(
        ...
        function (response) {
            ...
            if (page.endsWith("revisioni.php")) {
                ...
                page.endsWith("dettagli-targa.php") ||
                page.endsWith("dettagli-veicolo.php")
            ) {
            ...
            handleResponse(
                response,
                "Istanza inserita correttamente",
                callback
            );
        },
...
export function hideAddForm(event) {
    // when the form is submitted the fields are resetted
    if ($(".addFormDiv").is(":visible")) {
        $(".addFormDiv").toggle("slow");
...
function resetForm(excludeTarga = false) {
    if (excludeTarga) {
        // Store the value of the targa field before resetting
        var targaField = $("#addTarga");
        var targaValue = targaField.val();
        console.log(targaValue);
