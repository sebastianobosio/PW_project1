...
export function initializePage() {
    $(document).ready(function () {
        ...
        async function fetchRevisioneDetails(id) {
            console.log(id);
            try {
                const revisioneResponse = await new Promise(
                    (resolve, reject) => {
                        handleAjaxRequest(
                ...
                if (revisioneResponse.success == true) {
                    ...
                    $("#titolo").html(
                        "<h1>Dettagli sulla revisione " +
                            revisione.numero +
                            "</h1>"
                    );
                    const revisioneComponent = await renderRevisioneDetail(
                        revisione
                    ); // here i shoul render it with a different style component
                    ...
                    if (targaResponse.success == true) {
                        ...
                        $(".targa .titolo").html(
                            "<h3>Nessuna targa associata a questa revisione. Strano!</h3>"
                        );
